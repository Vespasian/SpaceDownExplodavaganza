...
func _process(delta):
	if dead:
		return
	if Input.is_action_pressed("player2_ui_left"):
		position.x -= speed * delta
	if Input.is_action_pressed("player2_ui_right"):
		position.x += speed * delta
	if Input.is_action_pressed("player2_ui_up"):
		position.y -= speed * delta
	if Input.is_action_pressed("player2_ui_down"):
		position.y += speed * delta
...
func _on_spaceship2_area_entered(area):
	if not "laser" in area.get_name() and not "spaceship" in area.get_name():
		emit_signal("player2_hit")
		area.queue_free()

func _on_health2_dead2():
	if dead:
		return
	dead = true
